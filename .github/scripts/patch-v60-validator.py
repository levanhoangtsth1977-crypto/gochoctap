...
# 1) Validator/Preview UI: idempotent insertion.
...
# 2) Add authoritative preflight + preview helpers once.
...
# 3) After scan/Gemini/approval, refresh downstream panels.
repls = {
    "audit('Quét hoàn tất: '+S.subjects.map(x=>LABEL[x]).join(', '))": "__v60Refresh();audit('Quét hoàn tất: '+S.subjects.map(x=>LABEL[x]).join(', '))",
    "render();$('gemStatus').textContent='✅ Gemini đã rà soát toàn bộ môn đã chọn · thêm '+added+' Change Set hợp lệ.'": "render();__v60Refresh();$('gemStatus').textContent='✅ Gemini đã rà soát toàn bộ môn đã chọn · thêm '+added+' Change Set hợp lệ.'",
...
# 4) Require a clean preflight before APPLY.
s = s.replace(
    "$('apply').onclick=()=>{let ok=0,fail=[];",
    "$('apply').onclick=()=>{const pf=__v60Refresh();if(!pf.approved||pf.invalid||pf.review){$('bulkStatus').textContent='🔒 Chưa thể áp dụng: '+pf.approved+' đã duyệt · '+pf.invalid+' lỗi · '+pf.review+' cần xác minh.';return}let ok=0,fail=[];"
)

# 5) Replace the old post-apply export gating with authoritative gating and explicit output message.
old_apply = "$('word').disabled=!!(fail.length||bad.length)||!ok;$('zip').disabled=!!(fail.length||bad.length)||!ok;$('log').disabled=!S.changes.length;$('preview').textContent=S.docs.map(d=>d.name+'\\n'+Object.entries(d.parts).map(([p,x])=>p+'\\n'+pars(x).map(a=>a.text).join('\\n')).join('\\n')).join('\\n\\n');$('bulkStatus').textContent='✏️ Đã áp dụng '+ok+' Change Set.';step(6);audit('Áp dụng '+ok+' thay đổi.')"
new_apply = "__v60Preview();const post=__v60Preflight();S.applied=ok>0&&fail.length===0&&bad.length===0&&post.invalid===0&&post.review===0;$('aa').textContent=ok;$('validator').textContent=S.applied?'✅ Validator sau áp dụng đạt · '+ok+' thay đổi đã được kiểm tra.':(fail.length||bad.length?'❌ Validator thất bại · '+fail.concat(bad.map(c=>c.id)).join(' | '):'⚠️ Chưa đạt điều kiện xuất.');$('word').disabled=!S.applied;$('zip').disabled=!S.applied;$('log').disabled=!S.changes.length;$('out').textContent=S.applied?'✅ Bản làm việc đã được áp dụng và kiểm tra. Có thể xuất Word/ZIP.':'🔒 Chưa thể xuất: phải Áp dụng và Validator đạt.';$('bulkStatus').textContent='✏️ Đã áp dụng '+ok+' Change Set.';step(S.applied?6:5);audit('Áp dụng '+ok+' thay đổi; Validator sau sửa: '+(S.applied?'ĐẠT':'CHƯA ĐẠT'))"
if old_apply in s:
    s = s.replace(old_apply, new_apply, 1)

# 6) Explicit Validator/Preview buttons before Gemini test.
...
# 7) Make export buttons self-explanatory if somehow clicked while gated.
# Keep real buttons disabled until S.applied, but provide an explicit status element.
if "S.applied?'✅ Bản làm việc đã được áp dụng" not in s:
    s = s.replace("$('out').textContent='Chưa có đầu ra.'", "$('out').textContent=S.applied?'✅ Bản làm việc đã sẵn sàng để xuất.':'🔒 Chưa có đầu ra: hãy Áp dụng Change Set đã duyệt trước.'", 1)

# 8) Scope/model sanity.
for bad in ('value="TIENG_ANH"','value="TIN_HOC"','value="MI_THUAT"','value="AM_NHAC"','gemini-3.6-flash','gemini-3.5-flash-lite'):
    if bad in s:
        raise SystemExit('LEGACY_TOKEN '+bad)
for req in ('id="validateNow"','id="previewNow"','function __v60Preflight()','function __v60Preview()','function __v60Refresh()'):
    if req not in s:
        raise SystemExit('MISSING '+req)

# 9) JavaScript syntax check artifact.
