...
# Insert authoritative preflight + preview helpers before apply handler.
if 'function __v60Preflight()' not in s:
    ...
    s=s[:idx]+block+s[idx:]
# Run preflight/preview immediately after scan and Gemini.
...
# Refresh immediately after individual and bulk approval/rejection.
...
# Apply requires a clean, fully-approved preflight. After apply, validate again and enable export only on success.
...
if old_apply in s:
    s=s.replace(old_apply,"__v60Preview();const post=__v60Preflight();S.applied=ok>0&&fail.length===0&&post.invalid===0;$('aa').textContent=ok;$('validator').textContent=post.invalid||post.review?'❌ Sau áp dụng: '+post.invalid+' lỗi · '+post.review+' cần xác minh.':'✅ Sau áp dụng: '+ok+' thay đổi đã được kiểm tra.';$('word').disabled=!S.applied;$('zip').disabled=!S.applied;$('log').disabled=!S.changes.length;$('bulkStatus').textContent='✏️ Đã áp dụng '+ok+' Change Set.';step(6);audit('Áp dụng '+ok+' thay đổi.')")
# Wire explicit validator and preview buttons.
...
# Ensure required downstream controls and no legacy models/subjects.
